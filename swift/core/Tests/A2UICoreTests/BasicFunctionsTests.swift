...
struct BasicFunctionsTests {
  ...
  @Test func regexFunctionEvaluatesCorrectly() throws {
    ...
    #expect(try fn.evaluate(arguments: ["value": .string("12345"), "pattern": .string("^[0-9]+$")]).boolValue == true)
    #expect(try fn.evaluate(arguments: ["value": .string("abc"), "pattern": .string("^[0-9]+$")]).boolValue == false)
  }

  @Test func lengthFunctionEvaluatesCorrectly() throws {
    let fn = LengthFunction()
    #expect(try fn.evaluate(arguments: ["value": .string("hello"), "min": .integer(2), "max": .integer(10)]).boolValue == true)
    #expect(try fn.evaluate(arguments: ["value": .string("h"), "min": .integer(2)]).boolValue == false)
    #expect(try fn.evaluate(arguments: ["value": .string("hello world"), "max": .integer(5)]).boolValue == false)
  }

  @Test func numericFunctionEvaluatesCorrectly() throws {
    let fn = NumericFunction()
    #expect(try fn.evaluate(arguments: ["value": .number(25), "min": .number(10), "max": .number(50)]).boolValue == true)
    #expect(try fn.evaluate(arguments: ["value": .number(5), "min": .number(10)]).boolValue == false)
    #expect(try fn.evaluate(arguments: ["value": .number(100), "max": .number(50)]).boolValue == false)
  }
  ...
  @Test func formatNumberFunctionEvaluatesCorrectly() throws {
    ...
    #expect(res.stringValue?.contains("1,234.57") == true || res.stringValue?.contains("1234.57") == true)
  }
  ...
  @Test func pluralizeFunctionEvaluatesCorrectly() throws {
    ...
    let oneRes = try fn.evaluate(arguments: ["value": .number(1), "one": .string("item"), "other": .string("items")])
    #expect(oneRes.stringValue == "item")

    let otherRes = try fn.evaluate(arguments: ["value": .number(5), "one": .string("item"), "other": .string("items")])
    #expect(otherRes.stringValue == "items")
  }

  @Test func logicalFunctionsEvaluateCorrectly() throws {
    let andFn = AndFunction()
    #expect(try andFn.evaluate(arguments: ["values": .array([.boolean(true), .boolean(true)])]).boolValue == true)
    #expect(try andFn.evaluate(arguments: ["values": .array([.boolean(true), .boolean(false)])]).boolValue == false)

    let orFn = OrFunction()
    #expect(try orFn.evaluate(arguments: ["values": .array([.boolean(false), .boolean(true)])]).boolValue == true)
    #expect(try orFn.evaluate(arguments: ["values": .array([.boolean(false), .boolean(false)])]).boolValue == false)
