...
def generate_controls(text: str, full_text: str = None) -> list:
    ...
    controls_schema = {
        ...
        "properties": {
            "initial_thought": {
                "type": "string",
                "description": (
                    "An insightful, evocative paragraph (1-2 short sentences) offering"
                    " creative direction, critique, or themes to inspire the writer."
                ),
            },
            "controls": {
                "type": "array",
                "description": (
                    "A list of 2 to 3 dynamic UI components best suited for tuning the"
                    " selected text."
                ),
                "items": {
                    "type": "object",
                    "properties": {
                        ...
                        "label": {
                            "type": "string",
                            "description": (
                                "Human readable label. For sliders MUST ALWAYS follow"
                                " 'X vs. Y' pattern."
                            ),
                        },
                        "options": {
                            ...
                            "items": {"type": "string"},
                            "description": (
                                "Required only when type='select'. The list of string"
                                " choices to populate dropdown."
                            ),
                        },
                    },
                    "required": ["type", "label"],
    ...
    try:
        ...
        summary = config_json.get(
            "initial_thought",
            "What direction would you like to explore with this segment?",
        )
    except Exception as e:
        print(f"Error calling Gemini for controls: {e}")
        controls = DEFAULT_CONTROLS
        summary = (
            "Ready to explore variations. Adjust the dials below to tune the output."
        )
    ...
    components = [
        {"id": "root", "component": {"Card": {"child": "col"}}},
        {
            ...
            "component": {
                "Text": {
                    "text": {"literalString": "Inspiration Agent"},
                    "usageHint": "h3",
                }
            },
        },
        {
            "id": "summary",
            "component": {
                "Text": {"text": {"path": "/summary_text"}, "usageHint": "body"}
            },
        },
    ...
    for i, ctrl in enumerate(controls):
        ...
        components.append(
            {"id": spacer_id, "component": {"Text": {"text": {"literalString": " "}}}}
        )
        ...
        if ctype == "checkbox":
            ...
            formatted_options = [
                {"label": {"literalString": o}, "value": o} for o in opt_raw
            ]
            lbl_id = f"lbl_{i}"
            ctrl_id = f"select_{i}"
    ...
    components.append(
        {"id": "spacer_end", "component": {"Text": {"text": {"literalString": " "}}}}
    )

    column_children.append("apply_btn")
    components.append({
        "id": "apply_btn_txt",
        "component": {"Text": {"text": {"literalString": "Generate Revision"}}},
    })
    components.append({
        "id": "apply_btn",
        "component": {
            "Button": {
                ...
                "action": {
                    ...
                    "context": [{
                        "key": "control_config_json",
                        "value": {"literalString": json.dumps(controls)},
                    }],
                },
            }
        },
    ...
    a2ui_messages = [
        {"dataModelUpdate": {"surfaceId": surface_id, "contents": data_model_contents}},
    ...
    for i, ctrl in enumerate(controls):
        ...
        if ctype == "checkbox":
            ...
            try:
                ...
                num_val = 0.5
            control_prompt_lines.append(
                f"- {label}: {num_val:.2f} (0 meaning low/minimum expression, 1 meaning"
                " high/maximum)"
            )
