...
const inputArg = argMap.input;
const outputArg = argMap.output;

if (!inputArg || !outputArg) {
  console.error('Usage: node inline.js --input <input_dir> --output <output_file>');
  process.exit(1);
}

const baseDir = process.cwd();
const buildDir = path.resolve(baseDir, inputArg);
const outputFile = path.resolve(baseDir, outputArg);
const outputDir = path.dirname(outputFile);

function getActualBuildDir(dir) {
  const rawDir = path.join(dir, 'browser');
  if (fs.existsSync(path.join(rawDir, 'index.html'))) return rawDir;
  if (fs.existsSync(path.join(dir, 'index.html'))) return dir;
  throw new Error(`Could not find index.html in ${dir} or ${rawDir}`);
}

try {
  const actualBuildDir = getActualBuildDir(buildDir);
  console.log(`Using build directory: ${actualBuildDir}`);

  let indexHtml = fs.readFileSync(path.join(actualBuildDir, 'index.html'), 'utf-8');
  ...
  indexHtml = indexHtml.replace(/<script[^>]*src="([^"]+)"[^>]*><\/script>/g, (match, src) => {
    const filePath = path.join(actualBuildDir, src);
    if (fs.existsSync(filePath)) {
      let content;
      if (src === 'main.js') {
        console.log(`Bundling and inlining JS: ${src}`);
        const bundledPath = path.join(actualBuildDir, 'main.bundled.js');
        try {
          execSync(
  ...
  indexHtml = indexHtml.replace(
    ...
    (match, href) => {
      const filePath = path.join(actualBuildDir, href);
      if (fs.existsSync(filePath)) {
