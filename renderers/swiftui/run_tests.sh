...
# Get swiftui root and repository root
SWIFTUI_ROOT="$(cd "$(dirname "$0")" && pwd)"
REPO_ROOT="$(cd "$SWIFTUI_ROOT/../.." && pwd)"
cd "$REPO_ROOT"

echo "=================================================="
echo "Running A2UI SwiftUI iOS Tests"
echo "=================================================="
...
SIM_LINE=$(xcodebuild -showdestinations -scheme A2UISwiftCore-Package 2>/dev/null \
  | grep "platform:iOS Simulator" | grep -m 1 "iPhone" || true)
...
if [ -n "$SIM_LINE" ]; then
  ...
  echo "Running tests..."
  xcodebuild test -scheme A2UISwiftCore-Package -destination "$SIMULATOR_DEST" \
